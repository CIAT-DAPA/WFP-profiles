...
calc_indices <- function(climate = infile,
  ...
  if(!file.exists(outfile)){
    ...
    clim_data <- clim_data %>% dplyr::select(-year) %>% tibble::as_tibble() %>%
      dplyr::mutate(id1 = id) %>%
            ...
            idx <- tbl_list %>%
              ...
              purrr::map(.f = function(df){
                idx <- tibble::tibble(ATR  = calc_atrMP(PREC = df$prec), 
                                      ...
                                      NWLD = calc_cNWLDMP(LOGG = df$LOGGING),
                                      NWLD50 = calc_cNWLD50MP(LOGG = df$LOGGING, sat = soilst),
                                      NWLD90 = calc_cNWLD90MP(LOGG = df$LOGGING, sat = soilst), 
