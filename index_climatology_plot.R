...
# Historic indices
h0 <- paste0(root,'/7.Results/',country,'/past/',iso,'_indices_monthly_old.fst') %>%
  tidyft::parse_fst(.) %>%
  base::as.data.frame()
h0$model <- 'Historical'
...
ff <- list.files(path = paste0(root,'/7.Results/',country,'/future/'), pattern = '.fst$', full.names = T, recursive = T) %>%
  grep(pattern = '_indices_monthly_old.fst$', x = ., value = T) %>%
...
complete <- all_data %>% filter( Index %in% vars )
