...
makePlotSingle <- function(admin,level,v,f,dissolve,pdir){
  
  cat("processing ", unique(v$NAME_0), "\n")
  ...
  onames <- paste(vs$pnames, collapse = "_")
...
makePlots <- function(i, eco, dir){
  ...
  # LZ boundary
  level <- ecos$level
  v <- shapefile(file.path(fdir, "LZ/regions", "hti_regions.shp"))
  vs <- aggregate(v, by=roi, sums=list(list(mean, 'LZNUM')))
  
  roi <- as.vector(vs@data[, roi])
  
  # list of crops
  crp <- ecos$value_chain
  
  datadir <- file.path(datadir, "result/2_5min")
  ...
  for (f in ff){
    cat("processing ", f, "\n")
    lapply(roi,makePlotSingle,level,vs,f,dissolve,pdir)
  }
}

#####################################################################################
# input
datadir <- "G:\\My Drive\\work\\ciat\\ecocrop"
eco <- readxl::read_excel("suitability/ecocrop_runs.xlsx", sheet = 3)

outdir <- "G:\\My Drive\\work\\ciat\\wfp"

# we will deal with the dissolve later
# ecod <- eco[eco$dissolve==0, ]

lapply(1:nrow(eco), makePlots, eco, dir)

# one
lapply(16, makePlots, eco, dir)

###################################################################################

# save results to powerpoint
dir <- "G:\\My Drive\\work\\ciat\\wfp"
dd <- list.dirs(dir, recursive = FALSE)
isol <- basename(dd[1])

for(iso in isol){
  # location to save ppt
  pptpath <- file.path(dir, iso, paste0(iso, "_suitability_analysis.pptx"))
  unlink(pptpath)
  print(pptpath)
  
  # list of file
  ff <- list.files(file.path(dir, iso, "png"), pattern = ".png", full.names = TRUE)
  
  if(length(ff)==0){next}
  
  # first setup individual files
  pat <- paste0('^.*',iso,'_\\s*|\\s*.png$')
  crps <- unique(gsub(pat, '', basename(ff)))
  
  admins <- stringr:: str_split(basename(ff), fixed("_suitability"))
  admins <- unique(unlist(lapply(admins, "[[", 1)))
  
  for(admin in admins){
    
    f1 <- grep(admin, ff, value = TRUE)
    
    for(crp in crps){
      
      f2 <- grep(crp, f1, value = TRUE)
      im1 <- grep("group", f2, value = TRUE)
      im2 <- grep("change", f2, value = TRUE)
      imgs <- c(im1,im2)
      
      for(img in imgs){
        create_pptx(img, pptpath, 6.5, 6.5)
      }
    }
  }
}
